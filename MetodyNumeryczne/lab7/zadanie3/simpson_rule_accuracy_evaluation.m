function [ft_5, integral_1000, Nt, integration_error] = simpson_rule_accuracy_evaluation()
    ...
    for i = 1:length(Nt)
        ...
        integral_approx = simpson_rule(x);
    ...
    title('zależność błędu całkowania od liczby podprzedziałów - metoda simsona');
    grid on;
...
function integral_approximation = simpson_rule(x)
    ...
    for i = 1:n
        a = x(i);
        b = x(i+1);
        fa = failure_density_function(a);
        fb = failure_density_function(b);
        fm = failure_density_function((a + b)/2);
        
        integral_approximation = integral_approximation + (b - a)/6 * (fa + 4*fm + fb);
    end
