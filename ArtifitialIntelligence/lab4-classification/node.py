...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split in possible_splits:
            left_spl, right_spl = y[:split], y[split:]

            left_neg, left_pos = np.sum(left_spl == 0), np.sum(left_spl == 1)
            right_neg, right_pos = np.sum(right_spl == 0), np.sum(right_spl == 1)

            left = left_neg + left_pos
            right = right_neg + right_pos
